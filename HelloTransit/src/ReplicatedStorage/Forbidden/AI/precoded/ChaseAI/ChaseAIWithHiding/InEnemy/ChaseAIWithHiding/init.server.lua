...
-- Returns true if the player is a possible target, hook any special things like isHiding, etc... here.
...
local function Chase(player: Player)
	...
	local function trackPlayer()
		if #config.standardPathfindSettings > 0 then 
			ai.SmartPathfind(config.enemy_char, player.Character, false, {Tracking = true, StandardPathfindingSettings = config.standardPathfindSettings, SMMD_RaycastParams = {range = 25, filterTable = {config.enemy_char, chasePart}}, Visualize = config.Visualize}) -- start player chase.
		else
			ai.SmartPathfind(config.enemy_char, player.Character, false, {Tracking = true, SMMD_RaycastParams = {range = 25, filterTable = {config.enemy_char, chasePart}}, Visualize = config.Visualize}) -- start player chase.
		end
	...
	while isChasing do
		...
		if std.math.LineOfSight(config.enemy_char, plr_hrt, {range = config.detectionRange, SeeThroughTransparentParts = config.seeThroughTransparent, filterTable = {config.enemy_char, chasePart}}) then
			...
			spawn(function()
				...
				if #config.standardPathfindSettings > 0 then 
					result = ai.SmartPathfind(config.enemy_char, chasePart, true, {SkipToWaypoint = 2, StandardPathfindingSettings = config.standardPathfindSettings, SMMD_RaycastParams = {range = 25}}) -- start player chase.
				else
					result = ai.SmartPathfind(config.enemy_char, chasePart, true, {SkipToWaypoint = 2, SMMD_RaycastParams = {range = 25}}) -- start player chase.
				end
...
local function Wander()
	...
	spawn(function()
		local function tryPathfind()
			...
			if #config.standardPathfindSettings > 0 then 
				return ai.SmartPathfind(config.enemy_char, randomLocation, true, {StandardPathfindingSettings = config.standardPathfindSettings, SMMD_RaycastParams = {range = 25, filterTable = {config.enemy_char, chasePart}}}) -- start player chase.
			else
				return ai.SmartPathfind(config.enemy_char, randomLocation, true, {SMMD_RaycastParams = {range = 25, filterTable = {config.enemy_char, chasePart}}}) -- start player chase.
			end
		end
